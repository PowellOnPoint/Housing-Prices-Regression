...
model_mlr3 = `(SalePrice | TotalSF + Neighborhood + OverallQual)`
model_mlr3_normal = `(SalePrice | TotalSF + NeighborhoodGroup + OverallQual)` After grouping Neighborhood into 4 groups based on median sales price.
...
create_kaggle_submission <- function(model, filename = "submission.csv") {
  test_pred <- predict(model, newdata = test, interval = "none")
  submission <- tibble(
    Id = test$Id,
    SalePrice = exp(test_pred)
  )
  write_csv(submission, here("submission", filename))
  cat("Submission saved to submission/", filename, "\n")
}
